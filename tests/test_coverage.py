...
from br_eli_mcp.coverage import build_coverage
from br_eli_mcp.server import br_coverage


def _coverage():
    fn = getattr(br_coverage, "fn", br_coverage)
...
def test_tool_writes_an_audit_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("BR_ELI_AUDIT_DIR", str(tmp_path))
    _coverage()
    lines = [
        line
    ...
    assert any("br_coverage" in line for line in lines), (
        "an audit entry was written but it does not name br_coverage"
    )
