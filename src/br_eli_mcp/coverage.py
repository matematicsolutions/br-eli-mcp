...
SOURCE = 'dadosabertos.camara.leg.br, normas.leg.br and DataJud'
...
_FAMILIES: list[dict] = [{'name': 'Legislation and bills (Camara)', 'tool': 'br_search_proposicoes / br_get_norma'}, {'name': 'Case law (STJ, TST, CARF, TCU)', 'tool': 'br_search_case_stj / br_search_case_tst / br_get_case_carf / br_search_case_tcu'}]

_GAPS: list[dict] = [{'id': 'BR-001', 'family': 'Case law (STJ, TST, CARF, TCU)', 'missing': 'STF is out of scope - it does not feed DataJud (confirmed by design, not an outage) and this server has no STF tool.', 'fallback': 'Use the STF portal directly; do not infer STF coverage from these tools.'}, {'id': 'BR-002', 'family': 'Legislation and bills (Camara)', 'missing': 'Planalto (planalto.gov.br) is out of scope - no confirmed mechanical rule maps a URN Lex to a Planalto URL and REFLEGIS serves a bot challenge.', 'fallback': 'Resolve the act on normas.leg.br, then open Planalto by hand if needed.'}, {'id': 'BR-003', 'family': 'Legislation and bills (Camara)', 'missing': 'Legal force on a given date is not evaluated. Text is returned as the source publishes it; this connector does not decide whether a provision is in force.', 'fallback': 'Check the validity or consolidation dates on the source record itself.'}]
