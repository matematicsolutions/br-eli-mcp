...
Pulls a tiny static ``br-runtime.json.gz`` from this repo's GitHub Release on
first tool call so a dead endpoint can be repointed (edit runtime/br-runtime.json
...
Opt-out: set ``BR_ELI_RUNTIME_URL=""`` to disable the fetch.
"""
...
MANIFEST_URL = os.environ.get(
    "BR_ELI_RUNTIME_URL",
    "https://github.com/matematicsolutions/br-eli-mcp/releases/latest/download/br-runtime.json.gz",
)
_CACHE_NAME = "br-runtime.json"
_TIMEOUT = 15
_USER_AGENT = "br-eli-mcp-runtime (+https://github.com/matematicsolutions/br-eli-mcp)"

_runtime: dict[str, Any] | None = None


def _cache_dir() -> Path:
    env = os.environ.get("BR_ELI_CACHE_DIR")
    if env:
        return Path(env).expanduser()
    return Path.home() / ".matematic" / "cache" / "br-eli"
